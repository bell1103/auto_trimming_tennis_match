...
def load_ball_positions(json_path):
    ...
    # Each item is [frame, x, y]
    x_ball = [item[1] if item[1] is not None else None for item in data]
    y_ball = [item[2] if item[2] is not None else None for item in data]
    return x_ball, y_ball

def main(model_path, input_json):
    x_ball, y_ball = load_ball_positions(input_json)

    # Initialize detector
    detector = BounceDetector(path_model=model_path)

    # Predict bounce frames
    bounce_frames = detector.predict(x_ball, y_ball)

    print("Predicted bounce frames:", sorted(bounce_frames))

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run bounce detection from JSON with [frame, x, y] format")
    parser.add_argument("--model", type=str, required=True, help="Path to CatBoost model (.cbm)")
    parser.add_argument("--input_json", type=str, required=True, help="Path to input JSON with [frame, x, y] format")
    args = parser.parse_args()

    main(args.model, args.input_json)
